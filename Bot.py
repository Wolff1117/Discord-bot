...
class command(commands.Cog):
    ...
    async def reload(self,ctx=None):
        ...
        for root,dirs,files in os.walk(fr"D:\haha funny image"): [hahaFunnyImages.append(f'{root}\\{filename}') for filename in files]  
        for root,dirs,files in os.walk(fr"D:\haha funny video"): [hahaFunnyVideos.append(f'{root}\\{filename}') for filename in files]
        for root,dirs,files in os.walk(fr"D:\haha funny sound"): [hahaFunnySounds.append(f'{root}\\{filename}') for filename in files]
    ...
    async def dm(self,ctx):
        await ctx.author.send('Hey, how was your day')
class help(commands.Cog):
    ...
    @help.command(name='comfort')
    async def help_comfort(self,ctx): await ctx.send(embed=discord.Embed(title='comfort',description='Sends something nice',color=0xf40bd5).add_field(name='Syntax',value='dip!comfort'))
